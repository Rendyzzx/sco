...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    if (opts["queque"] || db.data?.settings[this.user.jid]?.queque && m.text && !(isMods || isPrems)) {
      const id = m.id;
      this.msgqueque.add(id);
      await this.msgqueque.waitQueue(id);
    }
    if (opts["nyimak"]) return;
    if (opts["self"] && !isOwner && !isPrems) return;
...
- Anda dapat meng-upgrade menjadi user premium jika ingin menggunakan bot di private chat (pesan pribadi)`,
    ...
    if (m.isBaileys && m.sender === this.user?.jid) return;
        if (m.device === 'web') return;
        if (m.device === 'Desktop') return;
        if (m.device === 'desktop') return;
        ...
        case "remove":
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    //const welran = await WelcomeLeave(await this.profilePictureUrl(user, "image").catch(() => thumbs), await this.getName(user), "Welcome");
                    //const byeran = await WelcomeLeave(await this.profilePictureUrl(id, "image").catch(() => thumbs), await this.getName(id), "Leave");
                    const spaces = "                    "
                    ...
                    await this.reply(
                        ...
                        //{ key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(mek.chat ? { remoteJid: `status@broadcast` } : {}) }, message: { 'contactMessage': { 'displayName': 'Welcome Message', 'vcard': `BEGIN:VCARD\nVERSION:3.0\nN:XL;Welcome Message,;;;\nFN:Welcome Message,\nitem1.TEL;waid=${mek.sender.split('@')[0]}:${mek.sender.split('@')[0]}\nitem1.X-ABLabell:Ponsel\nEND:VCARD`, 'jpegThumbnail': this.getFile(isAddAction ? welran : byeran), thumbnail: this.getFile(isAddAction ? welran : byeran), sendEphemeral: true}}})
                }
            }
            break;
    case "promote":
      ...
      if (chat.detect) {
        await (conn || this).reply(id, promoteText.trim(), null)
      }
      break;
    case "demote":
...
export async function groupsUpdate(groupUpdates) {
  for (const groupUpdate of groupUpdates) {
    ...
    if (!text) continue;
    //await (conn || this).reply(id, text.trim(), null);
  }
}
...
global.dfail = async (type, m, conn) => {
  ...
  const msg = {
    owner: `Perintah ini hanya dapat digunakan oleh 
> Owner Bot!`,
    moderator: `Perintah ini hanya dapat digunakan oleh
> Moderator!`,
    premium: `Perintah ini hanya untuk member _*Premium*_ !
\n> ketik .buyprem untuk membeli premium Total user premium saat ini ${Object.values(global.db.data.users).filter(user => user.premium == true).length}`,
    ...
    error: `Fitur ini sedang *error/tidak bisa dipakai* !`,
