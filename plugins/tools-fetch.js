...
const fetchers = [{
  ...
  handler = async (m, { conn: conn, args: args, usedPrefix: usedPrefix, command: command }) => {
    try {
      ...
      if (!inputText) return m.reply(`Enter text or reply to the message with the text you want to process.`);
      const totalLinks = [...new Set([...inputText.matchAll(urlRegexSafe())].map((match => match[0])))];
      if (!totalLinks.length) return m.reply("Link error or invalid link.");
      m.react(waits)
