...
const handler = async (m, {
  ...
  try {
    ...
    {
      role: "user",
      content: "saya adalah "+m.name+" saya suka memanggil mu "+namebot+" tolong gunakan bahasa Indonesia dalam berbicara "+inputText
    }];
...
handler.before = async (m, {
  ...
  if (m.quoted?.id === keyId && m.text.trim()) {
    ...
    {
      role: "user",
      content: "saya adalah "+m.name+" saya suka memanggil mu "+namebot+" tolong gunakan bahasa Indonesia dalam berbicara "+m.text.trim()
    }];
      const data = await blackbox.chat(input, "Realtime", true, false, false, false);
      const answer = data.slice(data.lastIndexOf("$") + 1);
      if (answer) {
