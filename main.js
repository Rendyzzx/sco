...
if (global.pairingCode && !conn.authState.creds.registered) {
    ...
    let phoneNumber
    if (!!global.nomorbot) {
        phoneNumber = global.nomorbot.replace(/[^0-9]/g, '')

        if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Mulailah dengan kode WhatsApp negara Anda, contoh : global.nomorbotxxx")))
            process.exit(0)
        }
    } else {
...
async function connectionUpdate(update) {
    ...
    if (connection === "open") {
        console.log(chalk.bgGreen(chalk.black(`💃 ${namebot} telah aktif`)))        
    }
    if (connection == 'close') {
...
async function _quickTest() {
    ...
    setInterval(async () => {
        ...
        await clearTmp()
        conn.reply('0' + '@s.whatsapp.net', 'Sesi telah dibersihkan.', null) >
